...
template <class T, class Cmp> const T &TopN<T, Cmp>::peek_bottom() {
  // TODO: replace check
  // CHECK(!empty());
  if (state_ == UNORDERED) {
...
template <class T, class Cmp> std::vector<T> *TopN<T, Cmp>::Extract() {
...
template <class T, class Cmp> std::vector<T> *TopN<T, Cmp>::ExtractUnsorted() {
  auto out = new std::vector<T>;
  out->swap(elements_);
  if (state_ == HEAP_SORTED) {
...
template <class T, class Cmp> void TopN<T, Cmp>::Reset() {
  elements_.clear();
  state_ = UNORDERED;
}
