...
public:
  ...
  KenLMBeamScorer(Labels *labels, const char *kenlm_path, const char *trie_path)
      : lm_weight(1.0f), word_count_weight(0.0f),
  ...
  void ExpandStateEnd(KenLMBeamState *state) const {
    ...
    lm_score_delta += model
                          ->FullScore(state->model_state,
                                      model->GetVocabulary().EndSentence(), out)
                          .prob;
  ...
  void SetLMWeight(float lm_weight) { this->lm_weight = lm_weight; }
...
private:
  ...
  float ScoreIncompleteWord(const Model::State &model_state,
                            const std::wstring &word, Model::State &out) const {
