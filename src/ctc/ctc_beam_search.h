...
#ifndef CTC_CTC_BEAM_SEARCH_H_
#define CTC_CTC_BEAM_SEARCH_H_
...
public:
  ...
  template <typename Vector> void Step(const Vector &log_input_t);
...
Status CTCBeamSearchDecoder<CTCBeamState, CTCBeamComparer>::TopPaths(
  ...
  if (paths == nullptr || log_probs == nullptr) {
    return errors::FailedPrecondition("Internal paths are null");
  } else {
    paths->clear();
    log_probs->clear();
  }
