...
int generate_trie(Labels &labels, const char *kenlm_path,
  ...
  if (!ifs.is_open()) {
    std::cerr << "unable to open vocabulary" << std::endl;
    return -1;
  }
...
void set_kenlm_scorer_lm_weight(void *scorer, float weight) {
  ctc::KenLMBeamScorer *beam_scorer =
      static_cast<ctc::KenLMBeamScorer *>(scorer);
...
void *get_ctc_beam_decoder(int num_classes, int top_paths, int beam_width,
  ...
  case CTC_KENLM: {
    ctc::KenLMBeamScorer *beam_scorer =
        static_cast<ctc::KenLMBeamScorer *>(scorer);
    ctc::CTCBeamSearchDecoder<KenLMBeamState> *decoder =
        new ctc::CTCBeamSearchDecoder<KenLMBeamState>(num_classes, beam_width,
                                                      beam_scorer, blank_index,
                                                      merge_repeated == 1);
    return static_cast<void *>(decoder);
  }
  default:
...
int ctc_beam_decode(void *void_decoder, DecodeType type,
                    THFloatTensor *th_probs, THIntTensor *th_seq_len,
                    THIntTensor *th_output, THFloatTensor *th_scores,
                    THIntTensor *th_out_len) {
  ...
  // convert tensors to something the beam scorer can use sequence length
  int *seq_len_ptr = THIntTensor_data(th_seq_len);
  ptrdiff_t seq_len_offset = THIntTensor_storageOffset(th_seq_len);
  Eigen::Map<const Eigen::ArrayXi> seq_len(seq_len_ptr + seq_len_offset,
                                           batch_size);
  ...
  std::vector<Eigen::Map<Eigen::MatrixXf, 0,
                         Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>>>
      inputs;
  for (int t = 0; t < max_time; ++t) {
    inputs.emplace_back(probs_ptr + probs_offset + (t * probs_stride_0),
                        batch_size, num_classes,
                        Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>(
                            probs_stride_2, probs_stride_1));
  }

  // prepare/initialize output variables: paths, batches, class
  ...
  switch (type) {
  case CTC: {
    ctc::CTCBeamSearchDecoder<> *decoder =
        static_cast<ctc::CTCBeamSearchDecoder<> *>(void_decoder);
    scores = new Eigen::Map<Eigen::MatrixXf>(&score[0][0], batch_size,
                                             decoder->GetBeamWidth());
  ...
  case CTC_KENLM: {
    ctc::CTCBeamSearchDecoder<KenLMBeamState> *decoder =
        static_cast<ctc::CTCBeamSearchDecoder<KenLMBeamState> *>(void_decoder);
    scores = new Eigen::Map<Eigen::MatrixXf>(&score[0][0], batch_size,
                                             decoder->GetBeamWidth());
    Status stat = decoder->Decode(seq_len, inputs, &outputs, scores);
    if (!stat.ok()) {
  ...
  for (int p = 0; p < top_paths; ++p) {
    ...
    for (int b = 0; b < batch_size; ++b) {
      ...
      for (int64_t t = 0; t < num_decoded; ++t) {
        // TODO: this could be more efficient (significant pointer
        // arithmetic every time currently)
...
int generate_lm_trie(const char *label_str, int size, int blank_index,
                     int space_index, const char *lm_path,
                     const char *dictionary_path, const char *output_path) {
  std::wstring label_wstr;
  utf8::utf8to32(label_str, label_str + size, std::back_inserter(label_wstr));
  Labels labels(label_wstr.c_str(), utf8::distance(label_str, label_str + size),
                blank_index, space_index);
