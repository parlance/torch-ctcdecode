...
public:
  ...
  Status(error::Code code, std::stringPiece msg);
...
inline StatusStatus(const Status &s)
...
extern std::string *TfCheckOpHelperOutOfLine(const Status &v, const char *msg);
...
#define TF_CHECK_OK(val)                                                       \
  while (std::string *_result = TfCheckOpHelper(val, #val))                    \
  LOG(FATAL) << *(_result)
