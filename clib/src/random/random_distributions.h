...
#ifndef RANDOM_RANDOM_DISTRIBUTIONS_H_
#define RANDOM_RANDOM_DISTRIBUTIONS_H_

#include <algorithm>
#include <cmath>
#include <cstring>

#include "random/philox_random.h"
#include "unsupported/Eigen/CXX11/Tensor"

namespace random {

// Helper function to convert a 16-bit integer to a half between [0..1).
inline Eigen::half Uint16ToHalf(uint16 x);
...
public:
  ...
  inline ResultType operator()(Generator *gen) {
...
template <class Generator> class UniformDistribution<Generator, int32> {
public:
  // The number of elements that will be returned.
  static const int kResultElementCount = Generator::kResultElementCount;
  // Cost of generation of a single element (in cycles).
...
template <class Generator> class UniformDistribution<Generator, int64> {
public:
  // The number of elements that will be returned.
  static const int kResultElementCount = Generator::kResultElementCount / 2;
  // Cost of generation of a single element (in cycles).
...
public:
  ...
  inline explicit SingleSampleAdapter(Generator *gen)
...
inline void BoxMullerDouble(uint32 x0, uint32 x1, uint32 x2, uint32 x3,
                            double *d0, double *d1);
...
public:
  ...
  inline ResultType operator()(SingleSampleGenerator *gen) {
    ResultType results;
    int index = 0;
    while (true) {
...
#endif // RANDOM_RANDOM_DISTRIBUTIONS_H_
