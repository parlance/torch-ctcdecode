...
#ifndef RANDOM_PHILOX_RANDOM_H_
#define RANDOM_PHILOX_RANDOM_H_
...
public:
  ...
  inline explicit PhiloxRandom(uint64 seed) {
    key_[0] = static_cast<uint32>(seed);
    key_[1] = static_cast<uint32>(seed >> 32);
  }

  inline explicit PhiloxRandom(uint64 seed_lo, uint64 seed_hi) {
...
private:
  ...
  inline static void MultiplyHighLow(uint32 a, uint32 b, uint32 *result_low,
                                     uint32 *result_high) {
    *result_low = a * b;
  ...
  inline static ResultType ComputeSingleRound(const ResultType &counter,
                                              const Key &key) {
    uint32 lo0;
    uint32 hi0;
