...
#include "gtl/top_n.h"
#include "random/simple_philox.h"
...
TEST(TopNTest, MoveOnly) {
  ...
  for (int i = 0; i < 8; ++i)
    topn.push(StrPtr(new string(std::to_string(i))));
  for (int i = 8; i > 0; --i)
    topn.push(StrPtr(new string(std::to_string(i))));
